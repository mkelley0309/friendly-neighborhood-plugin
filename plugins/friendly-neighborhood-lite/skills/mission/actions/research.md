...
> **Phase council** (QRDPIV: R) — consult only when the concern is live (full map in `/friendly-neighborhood-lite:creed`): drivers `peter` (recall/docs/codebase), `noir` (investigation); counsel `jameson`; watch villains `mysterio`, `kingpin`.
...
Each pass is a separate subagent call. The subagent's job is **write findings to disk and return a short confirmation** — not to produce a finished summary for main context consumption. The named research drivers are `peter` (recall, docs, codebase lookups) and `noir` (investigation, root-cause) — spawn a pass as that Spider-variant (`subagent_type`) where it fits, falling back to a generic Sonnet subagent otherwise.
