...
> **Phase council** (QRDPIV: Cleanup) — consult only when the concern is live (full map in `/friendly-neighborhood-lite:creed`): driver `peter`; counsel `jameson`; watch villains `carnage`, `mysterio`.
