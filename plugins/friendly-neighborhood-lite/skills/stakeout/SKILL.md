...
4. **Run it.** `/loop <interval?> /friendly-neighborhood-lite:patrol` (or the specific step), or set a `schedule` routine for time-based runs.
...
1. **Intent + criteria.** State what the component is *for*, then derive its success criteria from that intent + the creed (semantic density, proportional ceremony, progressive disclosure, honest framing, accurate machinery).
2. **Review + improve.** `noir` diagnoses what's actually weak; `peter` applies the fix with **minimum force** — fix what's wrong, don't over-rewrite or gold-plate what isn't.
...
**Keep the loop honest:** watch `kingpin` (the over-polishing / wrong-target failure — another pass that just burns budget for no real gain) and `mysterio` (never call a component "better" without the gate confirming it).

Run it like any stakeout: `/loop /friendly-neighborhood-lite:stakeout` over a component list, one component per pass.
...
- **Villains on watch:** `carnage` (destructive unattended), `kingpin` (token burn over the long run), `mysterio` (declaring done without the gate).
