...
> **Phase council** (QRDPIV: P) — consult only when the concern is live (full map in `/friendly-neighborhood:creed`): driver `miguel`; counsel `uncle-ben`, `aunt-may`, `madame-web`; watch villains `octavius`, `kraven`, `kingpin`, `vulture`.
