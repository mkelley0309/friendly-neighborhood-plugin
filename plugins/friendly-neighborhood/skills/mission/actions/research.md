...
> **Phase council** (QRDPIV: R) — consult only when the concern is live (full map in `/friendly-neighborhood:creed`): drivers `silk` (recall/docs/codebase), `noir` (investigation); counsel `aunt-may`, `ned`, `madame-web`; watch villains `lizard`, `vulture`, `kraven`, `kingpin`.

Research is **mandatory** for every formal mission — QRDPIV and RPIV alike. There is no path from Questions (or Initiate) directly to Design or Plan without a research.md. If source material is provided by the user, it still passes through the research protocol: write it to `_working/`, then distill from `_working/`.
...
1. Confirm `_working/` exists in the patrol folder. Create it if not: it must exist before any pass runs.
2. Read `index.md` and `request.md` (if it exists). Do not read any other patrol file.
3. Determine which passes to run from the table above.

### Per Pass — Delegate to a Subagent (Sonnet)

Each pass is a separate subagent call. The subagent's job is **write findings to disk and return a short confirmation** — not to produce a finished summary for main context consumption. The named research drivers are `silk` (recall, docs, codebase lookups) and `noir` (investigation, root-cause) — spawn a pass as that Spider-variant (`subagent_type`) where it fits, falling back to a generic Sonnet subagent otherwise.
...
Run passes **sequentially, one at a time**. Do not run distillation until **all** pass files exist on disk. Certainty of completion takes priority over speed — a stalled or half-written pass file leaves the patrol in an unrecoverable state.
...
Immediately after `research.md` is accepted, write a queue signal to `knowledge/distillers/queue/{patrol}-{YYYYMMDD}.md`. This notifies the knowledge pipeline that research-phase material is available for vault candidacy review.

```yaml
signal: research-complete
workstream: {kebab-case-patrol-name}
...
{Distilled vault/sources knowledge relevant to this patrol — gaps noted explicitly}
