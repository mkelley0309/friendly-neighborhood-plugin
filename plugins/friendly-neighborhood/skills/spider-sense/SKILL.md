...
This manual protocol works alongside the harness's other safety layers:
...
- **`cat` (Black Cat) — security red-team.** When the change touches auth, secrets, input handling, or any external surface, this pre-flight isn't enough on its own — spawn `cat` for the deeper look. If she'd find a way in, close it before you proceed.
