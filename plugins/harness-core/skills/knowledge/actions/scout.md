...
Session-start health check for the knowledge pipeline. Reads the distiller queue, verifies referenced sources, registers first-party perspective inputs, updates queue status, and surfaces stale staging items.

## When to run

- **Default:** At the start of every session involving the `knowledge` skill — run Scout before any other action
...
- `control-plane/` or any workstream path
...
- If a queue file's `research_path` points to a control-plane workstream, Scout reads only enough to identify the source slug — it does not read or summarize the research content.
