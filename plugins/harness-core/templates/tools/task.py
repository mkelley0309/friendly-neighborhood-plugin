#!/usr/bin/env python3
"""Task (light unit) state management — lightweight, no phase machine, no LLM."""
...
TASKS_DIR = WORKSPACE / "control-plane" / "tasks"
PORTFOLIO_INDEX = TASKS_DIR / "index.md"
...
def task_dir(name):
    if not kebab_ok(name):
        die(f"invalid task name '{name}' — must be kebab-case (a-z, 0-9, hyphens); no path separators")
    p = TASKS_DIR / name
    if not p.exists():
        die(f"task '{name}' not found at {p}")
    return p

def read_index(name):
    p = task_dir(name) / "index.md"
    if not p.exists():
...
def ensure_portfolio_index():
    if not PORTFOLIO_INDEX.exists():
        ...
        PORTFOLIO_INDEX.write_text(
            "# Tasks\n\n"
            "## Active\n\n"
            "| Task | Description | Parent | Started |\n"
            "|---|---|---|---|\n\n"
            "## Complete\n\n"
...
*What this task is — what you understand the job to be, in a sentence or two. This understanding is the task's real value.*
...
*One line for the next task, if anyone picks up from here.*
...
def main():
    p = argparse.ArgumentParser(description="Task state management — lightweight")
    sub = p.add_subparsers(dest="cmd", required=True)

    sub.add_parser("list", help="List active tasks")

    s = sub.add_parser("create", help="Create a new task")
    s.add_argument("name", help="Task name (kebab-case)")
