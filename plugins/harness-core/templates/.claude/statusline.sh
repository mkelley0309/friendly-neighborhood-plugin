...
# Reads session JSON on stdin; scans control-plane for the active workstream + tier counts.
...
# Active workstream + phase (first active workstream).
if [ -d "$CP/workstreams" ]; then
  for f in "$CP"/workstreams/*/index.md; do
    ...
    if grep -qiE '^status:[[:space:]]*active' "$f"; then
      ...
      OUT="workstream:${name:-?}/${phase:-?}"
      break
    fi
  done
fi

# Counts: active objectives, workstreams, and tasks.
count_active() {
  local dir="$1" n=0
  if [ -d "$dir" ]; then
...
OBJ="$(count_active "$CP/objectives")"
WSC="$(count_active "$CP/workstreams")"
TASK="$(count_active "$CP/tasks")"

COUNTS="${OBJ} objectives / ${WSC} workstreams / ${TASK} tasks"
if [ -n "$OUT" ]; then
  OUT="$OUT | $COUNTS"
else
  OUT="$COUNTS"
